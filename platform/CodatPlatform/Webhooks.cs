    ...
    public class WebhooksSDK: IWebhooksSDK
    {
        ...
        public async Task<CreateRuleResponse> CreateAsync(CreateRule? request = null)
        {
            ...
            if((response.StatusCode == 200))
            {
                ...
                {
                    response.Rule = JsonConvert.DeserializeObject<Rule>(await httpResponse.Content.ReadAsStringAsync(), new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = new JsonConverter[] { new FlexibleObjectDeserializer() }});
                }
        ...
        public async Task<ListRulesResponse> ListAsync(ListRulesRequest? request = null)
        {
            ...
            if((response.StatusCode == 200))
            {
                if(Utilities.IsContentTypeMatch("application/json",response.ContentType))
                {
                    response.Rules = JsonConvert.DeserializeObject<Rules>(await httpResponse.Content.ReadAsStringAsync(), new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = new JsonConverter[] { new FlexibleObjectDeserializer() }});
                }
